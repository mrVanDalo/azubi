...
data Software = Package PackageName
              | RubyPackage PackageName
              | HaskellPackage PackageName
                deriving (Show)


data Target = FileTarget File
            | Folder
            | GitRepository
           deriving (Show)
...
class SystemConfiguration a where
  ...
  header a     = return ()
  exec _ state = putStrLn $ "not supported yet : " ++ (show state)
  footer a     = return ()

  mainProgramm ::  a -> [ State ] -> IO ()
  mainProgramm a states = do
    header a
    unwrap' $ map (exec a) states
    footer a
